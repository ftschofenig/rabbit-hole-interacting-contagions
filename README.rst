...
The committed PNG files in ``figures_final`` are the reference versions used
by the current manuscript. Running the notebooks regenerates them from the
public inputs.
...
copies to ``executed_notebooks`` and leave the committed source notebooks
clean. Each kernel runs with the source notebook directory as its working
directory, which is required by the relative paths in the notebooks.
